...
caldavtester-test-suite:
	...
	nix develop path:.#caldavtester -c bash -lc 'cd caldavtester-lab && ./bootstrap.sh >/dev/null && source ./.env-py2.sh && cd ccs-caldavtester && python3 -c "from pathlib import Path; p = Path(\"scripts/server/serverinfo.xml\"); t = p.read_text(); p.write_text(t.replace(\"<feature>sync-report-home</feature>\", \"<!-- <feature>sync-report-home</feature> -->\"))" && python2 testcaldav.py CalDAV/current-user-principal.xml CalDAV/propfind.xml CalDAV/put.xml CalDAV/get.xml CalDAV/delete.xml CalDAV/conditional.xml CalDAV/reports.xml CalDAV/sync-report.xml CalDAV/recurrenceput.xml CalDAV/floating.xml CalDAV/timezoneservice.xml CalDAV/timezonestdservice.xml CalDAV/implicittodo.xml'
