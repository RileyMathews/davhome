...
setup-integration-fixtures:
	...
	uv run python manage.py shell --settings=config.settings_test -c "from django.contrib.auth.models import User; from calendars.models import Calendar; users=['admin','apprentice','superuser']+[f'user{i:02d}' for i in range(1,41)]; [((lambda user, username: (user.set_password(username), user.save(update_fields=['password','is_active','email']), Calendar.objects.get_or_create(owner=user, slug='calendar', defaults={'name':'calendar','timezone':'UTC'}), Calendar.objects.get_or_create(owner=user, slug='tasks', defaults={'name':'tasks','timezone':'UTC'}), Calendar.objects.get_or_create(owner=user, slug='inbox', defaults={'name':'inbox','timezone':'UTC'}), Calendar.objects.get_or_create(owner=user, slug='outbox', defaults={'name':'outbox','timezone':'UTC'})))(User.objects.update_or_create(username=username, defaults={'email': f'{username}@example.com', 'is_active': True})[0], username)) for username in users]"
