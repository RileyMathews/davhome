...
class Property(PropertyBase):
    ...
    def _postCreateValue(self, value_type):
        ...
        if value_type in (
            Value.VALUETYPE_TIME,
            Value.VALUETYPE_DATETIME,
        ) and self.hasParameter(definitions.cICalParameter_TZID):
    ...
    def _init_attr_value_datetime(self, dt):
        ...
        if not dt.isDateOnly() and dt.local():
            ...
            self.mParameters.setdefault(definitions.cICalParameter_TZID, []).append(
                Parameter(
                    name=definitions.cICalParameter_TZID, value=dt.getTimezoneID()
                )
            )
    ...
    def _init_attr_value_datetimelist(self, dtl):
        ...
        if (len(dtl) > 0) and not dtl[0].isDateOnly() and dtl[0].local():
            ...
            self.mParameters.setdefault(definitions.cICalParameter_TZID, []).append(
                Parameter(
                    name=definitions.cICalParameter_TZID, value=dtl[0].getTimezoneID()
                )
            )
